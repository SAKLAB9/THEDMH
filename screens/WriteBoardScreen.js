...
export default function WriteBoardScreen({ navigation, route }) {
  const { university } = useUniversity();
  const { getConfig, getColorConfig } = useAppConfig();
  const config = { getColorConfig };
  const categories = [
    getConfig('board_tab1'),
    getConfig('board_tab2'),
    getConfig('board_tab3'),
    getConfig('board_tab4')
  ].filter(cat => cat); // 빈 값 제거
  const defaultCategory = categories[0] || '';
  const { category: initialCategory = defaultCategory, editBoard, selectedChannel } = route?.params || {};
  ...
  // 랜덤 동물 이모티콘 목록
  const animalEmojis = ['🐶', '🐱', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮', '🐷', '🐸', '🐵'];
  const getRandomAnimal = () => animalEmojis[Math.floor(Math.random() * animalEmojis.length)];
  const [author, setAuthor] = useState(getRandomAnimal());
  ...
  useEffect(() => {
    if (editBoard) {
      setIsEditMode(true);
      setEditBoardId(editBoard.id);
      setTitle(editBoard.title || '');
      setAuthor(editBoard.nickname || getRandomAnimal());
      setUrl(editBoard.url || '');
      setSelectedCategory(editBoard.category || initialCategory);
      
      // contentBlocks 초기화
      let blocks = [];
      
      // content_blocks가 있으면 사용
      if (editBoard.content_blocks && editBoard.content_blocks.length > 0) {
        // content_blocks를 복사하고 각 블록에 고유 ID가 없으면 생성
        blocks = editBoard.content_blocks.map((block, index) => {
          if (!block.id) {
            return {
              ...block,
              id: block.type === 'image' 
      ...
      if (!hasImageInContentBlocks && editBoard.images && Array.isArray(editBoard.images) && editBoard.images.length > 0) {
        // images 배열의 이미지를 contentBlocks에 추가
        const imageBlocks = editBoard.images.map((imageUrl, index) => ({
          type: 'image',
          uri: imageUrl,
  ...
  // 이미지를 base64로 변환하여 서버에 업로드 (게시판용)
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/upload-image`, {
        ...
        body: JSON.stringify({
          ...
          filename: `board_${Date.now()}_${Math.random().toString(36).substr(2, 9)}.jpg`,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // normalizedUniversity 정의
      const normalizedUniversity = targetUniversity.toLowerCase();

      // API 서버로 전송 (수정 모드면 PUT, 아니면 POST)
      const apiUrl = isEditMode 
        ? `${API_BASE_URL}/api/posts/${editBoardId}`
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          university: normalizedUniversity,
          nickname: author && author.trim() ? author.trim() : getRandomAnimal(),
      ...
      let errorMessage = error.message || '게시글 저장 중 오류가 발생했습니다.';
    ...
    <View className="flex-1" style={{ backgroundColor: colors.primary }}>
      ...
      <View className="flex-1 bg-white" style={{ marginTop: 70, ...(Platform.OS === 'web' ? { display: 'flex', flexDirection: 'column', height: 'calc(100vh - 130px)', overflow: 'hidden' } : {})}}>
        <View className="flex-row items-center justify-between px-6 border-b border-gray-200" style={{ height: 60, paddingTop: 5, paddingBottom: 5, ...(Platform.OS === 'web' ? { flexShrink: 0 } : {})}}>
          <Text className="text-xl font-bold" style={{ color: colors.primary }}>{isEditMode ? '게시판 수정' : '게시판 글쓰기'}</Text>
          <TouchableOpacity
            onPress={() => {
              if (selectedChannel) {
                navigation.navigate('Main', { screen: 'Board', params: { selectedChannel } });
              } else {
                if (navigation.canGoBack()) {
                navigation.goBack();
              } else {
