...
export default function CirclesScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    const loadPartnersImageUrls = async () => {
      ...
      // Supabase Storage에서 직접 이미지 URL 가져오기 (슬롯 크기에 맞춰 압축)
  ...
  useEffect(() => {
    const loadMiuhubToggle = async () => {
      try {
        const saved = await AsyncStorage.getItem('circles_miuhub_toggle_enabled');
        if (saved !== null) {
            ...
            <View className="flex-row items-center justify-between mb-4">
              ...
              <TouchableOpacity
                onPress={async () => {
                  ...
                  await AsyncStorage.setItem('circles_miuhub_toggle_enabled', String(newValue));
