...
export default function WriteNoticeScreen({ navigation, route }) {
  ...
  const categories = [
    getConfig('notice_tab1'),
    getConfig('notice_tab2'),
    getConfig('notice_tab3')
  ].filter(cat => cat); // 빈 값 제거
  const defaultCategory = categories[0] || '';
  const { category: initialCategory = defaultCategory, editNotice } = route?.params || {};
  // "전체"가 전달되거나 categories에 없는 경우 기본값 사용
  const validCategory = categories.includes(initialCategory) ? initialCategory : defaultCategory;
  ...
  useEffect(() => {
    if (editNotice) {
      setIsEditMode(true);
      setEditNoticeId(editNotice.id);
      setTitle(editNotice.title || '');
      setNickname(editNotice.nickname || '만든다');
      setUrl(editNotice.url || '');
      setSelectedCategory(editNotice.category || initialCategory);
      
      // contentBlocks 초기화
      let blocks = [];
      
      // content_blocks가 있으면 사용
      if (editNotice.content_blocks && editNotice.content_blocks.length > 0) {
        // content_blocks를 복사하고 각 블록에 고유 ID가 없으면 생성
        blocks = editNotice.content_blocks.map((block, index) => {
          if (!block.id) {
            return {
              ...block,
              id: block.type === 'image' 
      ...
      if (!hasImageInContentBlocks && editNotice.images && Array.isArray(editNotice.images) && editNotice.images.length > 0) {
        // images 배열의 이미지를 contentBlocks에 추가
        const imageBlocks = editNotice.images.map((imageUrl, index) => ({
          type: 'image',
          uri: imageUrl,
  ...
  const handleSubmit = async () => {
    ...
    const correctPassword = getCategoryPassword(university, selectedCategory, noticeTab1);
    ...
    try {
      ...
      let errorMessage = error.message || '공지사항 저장 중 오류가 발생했습니다.';
    ...
    <View className="flex-1" style={{ backgroundColor: colors.primary }}>
      ...
      <View className="flex-1 bg-white" style={{ marginTop: 70, ...(Platform.OS === 'web' ? { display: 'flex', flexDirection: 'column', height: 'calc(100vh - 130px)', overflow: 'hidden' } : {})}}>
        <View className="flex-row items-center justify-between px-6 border-b border-gray-200" style={{ height: 60, paddingTop: 5, paddingBottom: 5, ...(Platform.OS === 'web' ? { flexShrink: 0 } : {})}}>
          <Text className="text-xl font-bold" style={{ color: colors.primary }}>{isEditMode ? '공지사항 수정' : '공지사항 작성'}</Text>
          <TouchableOpacity
            onPress={() => {
        ...
        {(() => {
          const scrollViewContent = (
            <>
              ...
              <Text className="text-base font-semibold text-gray-900 mb-2">{getConfig('notice_write_url_label', 'RSVP (Optional)')}</Text>
              <TextInput
                className="border border-gray-300 rounded-lg text-base bg-white mb-4"
