...
if (USE_DATABASE) {
  try {
    ...
    console.error('[API Life Events Detail] 데이터베이스 연결 실패:', error);
  }
}
  ...
  try {
    ...
    if (USE_DATABASE && pool) {
      try {
        ...
        console.error('[API Life Events Detail] 데이터베이스 오류:', error);
    ...
    console.error('[API Life Events Detail] 일반 오류:', error);
    return res.status(500).json({ error: '서버 오류가 발생했습니다.', message: error.message });
  }
};
