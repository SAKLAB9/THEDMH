      ...
      try {
        ...
        try {
          ...
          try {
            ...
            console.error(`[공지사항 등록] 시퀀스 재설정 실패:`, seqError.message, seqError.stack);
          }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // 시퀀스 재설정 실패해도 계속 진행 (시퀀스가 없을 수도 있음)
            console.error(`[Life Events 등록] 시퀀스 재설정 실패:`, seqError.message, seqError.stack);
          }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // 시퀀스 재설정 실패해도 계속 진행 (시퀀스가 없을 수도 있음)
            console.error(`[Circles 등록] 시퀀스 재설정 실패:`, seqError.message, seqError.stack);
          }
      ...
      if (block.type === 'image' && block.uri) {
        if (block.uri.startsWith('data:image')) {
          ...
          try {
            ...
            console.error(`[Circles 수정] contentBlocks 이미지 저장 실패:`, error);
            return block;
          }
        }
      }
    ...
    for (let i = 0; i < imagesArray.length; i++) {
      ...
      if (imageData && imageData.startsWith('data:image')) {
        ...
        try {
          ...
          console.error(`[Circles 수정] 이미지 저장 실패:`, error);
        }
      } else if (imageData && imageData.startsWith('http')) {
        // 이미 업로드된 이미지 URL
        newImageUrls.push(imageData);
    ...
    if (USE_DATABASE && pool) {
      try {
        ...
        if (since) {
          const sinceTimestamp = parseInt(since, 10);
          if (!isNaN(sinceTimestamp)) {
            whereConditions.push(`EXTRACT(EPOCH FROM p.created_at) * 1000 > $${paramIndex}`);
            params.push(sinceTimestamp);
            paramIndex++;
          }
        }
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            // 시퀀스 재설정 실패해도 계속 진행 (시퀀스가 없을 수도 있음)
            console.error(`[게시판 등록] 시퀀스 재설정 실패:`, seqError.message, seqError.stack);
          }
